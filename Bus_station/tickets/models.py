...
class Trip(models.Model):
    ...
    company = models.ForeignKey(TransportCompany, on_delete=models.CASCADE, null=True)  # Связь с компанией, допускает NULL
...
class Order(models.Model):
    ...
    ticket_number = models.CharField(max_length=36, unique=True, default=uuid.uuid4, null=True)  # Уникальный номер билета

    def __str__(self):
        return f"Order {self.ticket_number} by {self.user.username} for {self.trip}"